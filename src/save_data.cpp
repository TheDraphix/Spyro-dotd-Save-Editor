...
// all offsets are relative to the ps3 version of the game unless otherwise stated.
{
	int offset = 0x0;
	int slots = 1;
	...
	for (int i=0; i<slots; i++)
	{
		...
		this->content.slot[i].game_state_uuid.value =		reinterpret_cast<uint32_t*>(&this->data[0x0021 + offset]);
		...
		else
		{
			...
			this->content.slot[i].spawn_point_uuid.value = reinterpret_cast<uint32_t*>(&this->data[0x0039 + offset]);
		}
		...
		this->content.slot[i].spyro_elem_exp =				reinterpret_cast<int32_t *>(&this->data[0x03e1 + offset]);
		this->content.slot[i].cynder_elem_exp =				reinterpret_cast<int32_t *>(&this->data[0x03f5 + offset]);
		...
		this->content.slot[i].spyro_mana_spent =				reinterpret_cast<uint32_t *>(&this->data[0x04be + offset]);
		this->content.slot[i].cynder_mana_spent =				reinterpret_cast<uint32_t *>(&this->data[0x04d2 + offset]);
		...
		this->content.slot[i].unknown_0x0538 = reinterpret_cast<uint8_t *>(&this->data[0x0538 + offset]);
		...
		this->content.slot[i].unknown_0x054a = reinterpret_cast<uint8_t *>(&this->data[0x054a + offset]);
	}
}
